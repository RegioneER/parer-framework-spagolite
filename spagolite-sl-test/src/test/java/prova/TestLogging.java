...
import it.eng.parer.sacerlog.ejb.util.LogOggettoRuolo;
...
public class TestLogging {
    ...
    @Test
    public void log_ok() throws Exception {
        userTransaction.begin();
        LogEvento logEvento = sacerLog.log(null, NOME_APPLICAZIONE, NOME_UTENTE, NOME_AZIONE, NOME_TIPO_OGGETTO,
                ID_OGGETTO_PRINCIPALE, NOME_FINESTRA);
        Assert.assertNotNull(logEvento);
        Assert.assertNotNull(logEvento.getIdEvento());
        Assert.assertEquals(NOME_AZIONE, logEvento.getNmAzione());
        Assert.assertEquals(NOME_FINESTRA, logEvento.getNmGeneratoreAzione());
     ...
     * userTransaction.begin(); // // Oggetti per SACER_IAM // // Setta la data aad un anno fa!! Date appoData=new
     * Date(); appoData=DateUtils.setYears(appoData, 2016); Timestamp tempoIndietro=new Timestamp(appoData.getTime());
     * LogEventoByScript logScript=new LogEventoByScript(); logScript.setDtRegEvento(tempoIndietro);
     ...
     * logScript.setIdApplic(new BigDecimal(321)); // Inserisce un record di script // Cambia leggermente la data
     * sacerLogHelper.insertEntity(logScript, true); appoData=DateUtils.setYears(appoData, 2016);
     * appoData=DateUtils.setMonths(appoData, 10);
     * 
     * tempoIndietro=new Timestamp(appoData.getTime()); LogEventoByScript logScript2=new LogEventoByScript();
     * logScript2.setDtRegEvento(tempoIndietro); logScript2.setIdAgente(new BigDecimal(423));
     ...
     * userTransaction.begin(); // Logga un evento da finestra di un utente // sacerLog.log("SACER_IAM",
     * "admin_generale", "toolbar/update", "Utente", new BigDecimal(5632),
     * "/amministrazioneUtenti/dettaglioUtenteWizard"); userTransaction.commit(); userTransaction.begin(); // cancella
     * un record script // sacerLogHelper.removeEntity(logScript, true); // sacerLogHelper.removeEntity(logScript2,
     * true); userTransaction.commit(); Assert.assertTrue(true); }
     */
    /*
     * @Test public void inserimentoDueRecordPerIamESacer() throws Exception { userTransaction.begin(); // // Oggetti
     * per SACER_IAM // // Setta la data aad un anno fa!! Date appoData=new Date(); Timestamp tempoIndietro=null;
     * LogEventoByScript logScript=null; LogEventoByScript logScript2=null; LogEventoByScript logScript2_1=null;
     * LogEventoByScript logScript3=null; LogEventoByScript logScript3_1=null; LogEventoByScript logScript4=null;
     * appoData=DateUtils.setYears(appoData, 2016); tempoIndietro=new Timestamp(appoData.getTime()); logScript=new
     * LogEventoByScript(); logScript.setDtRegEvento(tempoIndietro); logScript.setIdAgente(new BigDecimal(423));
     * logScript.setIdAzioneCompSw(new BigDecimal(64)); logScript.setIdOggetto(new BigDecimal(6320));
     * logScript.setIdTipoOggetto(new BigDecimal(46)); logScript.setTiRuoloAgenteEvento("executing program");
     * logScript.setTiRuoloOggettoEvento("outcome"); logScript.setIdApplic(new BigDecimal(321));
     * sacerLogHelper.insertEntity(logScript, true);
     * 
     * appoData=DateUtils.setYears(appoData, 2016); appoData=DateUtils.setMonths(appoData, 10); tempoIndietro=new
     * Timestamp(appoData.getTime()); logScript2=new LogEventoByScript(); logScript2.setDtRegEvento(tempoIndietro);
     * logScript2.setIdAgente(new BigDecimal(423)); logScript2.setIdAzioneCompSw(new BigDecimal(64));
     * logScript2.setIdOggetto(new BigDecimal(6320)); logScript2.setIdTipoOggetto(new BigDecimal(46));
     * logScript2.setTiRuoloAgenteEvento("executing program"); logScript2.setTiRuoloOggettoEvento("outcome");
     * logScript2.setIdApplic(new BigDecimal(321)); sacerLogHelper.insertEntity(logScript2, true);
     * 
     * appoData=DateUtils.setYears(appoData, 2016); appoData=DateUtils.setMonths(appoData, 11); tempoIndietro=new
     * Timestamp(appoData.getTime()); logScript2_1=new LogEventoByScript(); logScript2_1.setDtRegEvento(tempoIndietro);
     * logScript2_1.setIdAgente(new BigDecimal(423)); logScript2_1.setIdAzioneCompSw(new BigDecimal(64));
     * logScript2_1.setIdOggetto(new BigDecimal(6320)); logScript2_1.setIdTipoOggetto(new BigDecimal(46));
     * logScript2_1.setTiRuoloAgenteEvento("executing program"); logScript2_1.setTiRuoloOggettoEvento("outcome");
     * logScript2_1.setIdApplic(new BigDecimal(321)); // Inserisce un record di script
     * sacerLogHelper.insertEntity(logScript2_1, true);
     * 
     * // // Oggetti per SACER // // Setta la data ad un anno fa!! appoData=new Date();
     * appoData=DateUtils.setYears(appoData, 2015); tempoIndietro=new Timestamp(appoData.getTime()); logScript3=new
     * LogEventoByScript(); logScript3.setDtRegEvento(tempoIndietro); logScript3.setIdAgente(new BigDecimal(423));
     * logScript3.setIdAzioneCompSw(new BigDecimal(67)); logScript3.setIdOggetto(new BigDecimal(7984));
     * logScript3.setIdTipoOggetto(new BigDecimal(25)); logScript3.setTiRuoloAgenteEvento("executing program");
     * logScript3.setTiRuoloOggettoEvento("outcome"); logScript3.setIdApplic(new BigDecimal(322)); // Inserisce un
     * record di script // Cambia leggermente la data sacerLogHelper.insertEntity(logScript3, true);
     * 
     * appoData=DateUtils.setYears(appoData, 2016); appoData=DateUtils.setMonths(appoData, 10); tempoIndietro=new
     * Timestamp(appoData.getTime()); logScript4=new LogEventoByScript(); logScript4.setDtRegEvento(tempoIndietro);
     * logScript4.setIdAgente(new BigDecimal(423)); logScript4.setIdAzioneCompSw(new BigDecimal(67));
     * logScript4.setIdOggetto(new BigDecimal(7984)); logScript4.setIdTipoOggetto(new BigDecimal(25));
     * logScript4.setTiRuoloAgenteEvento("executing program"); logScript4.setTiRuoloOggettoEvento("outcome");
     * logScript4.setIdApplic(new BigDecimal(322)); // Inserisce un record di script
     * sacerLogHelper.insertEntity(logScript4, true);
     * 
     * appoData=DateUtils.setYears(appoData, 2016); appoData=DateUtils.setMonths(appoData, 11); tempoIndietro=new
     * Timestamp(appoData.getTime()); logScript3_1=new LogEventoByScript(); logScript3_1.setDtRegEvento(tempoIndietro);
     * logScript3_1.setIdAgente(new BigDecimal(423)); logScript3_1.setIdAzioneCompSw(new BigDecimal(67));
     * logScript3_1.setIdOggetto(new BigDecimal(7984)); logScript3_1.setIdTipoOggetto(new BigDecimal(25));
     * logScript3_1.setTiRuoloAgenteEvento("executing program"); logScript3_1.setTiRuoloOggettoEvento("outcome");
     * logScript3_1.setIdApplic(new BigDecimal(322)); // Inserisce un record di script
     * sacerLogHelper.insertEntity(logScript3_1, true);
     * 
     * userTransaction.commit();
     * 
     * 
     * // sacerLog.log("SACER", "admin_generale", "toolbar/update", "Tipo documento", new BigDecimal(7984),
     * "/amministrazioneUtenti/dettaglioUtenteWizard");
     * 
     * 
     * Assert.assertTrue(true); }
     */
    /*
     * @Test public void inserimentoDueRecordPerIamESacer2() throws Exception {
     * 
     * userTransaction.begin(); // // Oggetti per SACER_IAM // // TEST di modifica dell'utente admin_generale di cui
     * però si inserisce un log by script del 2016 // che viene loggato prima, ed in una nuova transazione. Date
     * appoData=new Date(); Timestamp tempoIndietro=null; LogEventoByScript logScript=null;
     * appoData=DateUtils.setYears(appoData, 2016); tempoIndietro=new Timestamp(appoData.getTime()); logScript=new
     * LogEventoByScript(); logScript.setDtRegEvento(tempoIndietro); // setta data nel 2016 logScript.setIdAgente(new
     * BigDecimal(423)); // Script saceriam logScript.setIdAzioneCompSw(new BigDecimal(64)); // Modifica con script
     * logScript.setIdOggetto(new BigDecimal(5000)); // admin_generale logScript.setIdTipoOggetto(new BigDecimal(46));
     * // tipo Utente logScript.setTiRuoloAgenteEvento("executing program");
     * logScript.setTiRuoloOggettoEvento("outcome"); logScript.setIdApplic(new BigDecimal(321)); // sacer_iam
     * logScript.setDsMotivoScript("TEST TRANSACTION ID DIFFERENTE!"); sacerLogHelper.insertEntity(logScript, true);
     * userTransaction.commit(); Assert.assertTrue(true); }
     */

    /*
     * @Test public void _9901_chiamataAllAPIDiLogging_Comp_e_normale() throws Exception { userTransaction.begin();
     * sacerLog.log(null, "SACER_IAM", "Servizio Allinea ruolo", "Modifica per allineamento ruolo", "Ruolo", new
     * BigDecimal(65021642), "ALLINEA_RUOLO", PremisEnums.TipoAgenteEvento.EXECUTING_PROGRAM); userTransaction.commit();
     ...
     */
    /*
     * @Test public void _990_chiamataAllTest_Massivo() throws Exception {
     * 
     * userTransaction.begin(); // sacerLogHelper.getAgenteByNomeAgente("admin_generale"); // Query
     * q=entityManager.createNamedQuery("LogEvento.findAll"); Query
     * q=entityManager.createNamedQuery("LogEventoPippo.findAllIds"); List<Long> l=q.getResultList(); for (Long id : l)
     * { log.info(String.format("id efvento=%d",id)); ejbTest.operazioneInNuovatgransazione(id); }
     * userTransaction.commit(); Assert.assertTrue(true); }
     * 
     */
}
