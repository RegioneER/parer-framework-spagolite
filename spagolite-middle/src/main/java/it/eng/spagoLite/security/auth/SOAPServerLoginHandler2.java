...
/**
 * Handler SOAP per un WS Server generico: effettua il login (autenticazione con user/password)
 * dell'utente e verifica che l'utente sia autorizzato ad eseguire il servizio su cui l'handler �
 * applicato per almeno un'organizzazione. E' utile per i servizi che sono trasversali alle
 * organizzazioni. Si aspetta che i campi user e password siano nell'header come da standard ws
 * security.
 *
 *
 * @author Quaranta_M
...
public class SOAPServerLoginHandler2 implements SOAPHandler<SOAPMessageContext> {
    ...
    private static final String X_FORWARDED_UNDEFINED = "l'header http x-forwarded non � definito";
	...
	if (!outbound) {
		...
		if (usernameEl != null && passwordEl != null && svcn != null
		    ...
		    WSLoginHandler.loginAndCheckAuthzAtLeastOneOrganiz(username, password,
			    servizioWeb, ipAddress, em, false);
