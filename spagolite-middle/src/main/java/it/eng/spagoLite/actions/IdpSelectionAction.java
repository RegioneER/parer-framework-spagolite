...
public class IdpSelectionAction {
    ...
    public String handler(Model model, HttpServletRequest request) {
        /*
         * Anche questa è un workaround perché all'atto del logout da keycloak il logout veniva fatto ma mi
         * reindirizzava dono un http 302 verso /discovery?error !!!
         *
         * Quindi intercettando il parametro che contiene "" capisco che devo mandarlo nella pagina finale del logout
         * effettuato correttamente. E funziona.
         */
        ...
        if (paramError != null && paramError.equals("")) {
            ...
            if (refreshableRelyingPartyRegistrationRepository == null) {
                ...
                refreshableRelyingPartyRegistrationRepository.forEach(r -> {
                    LOGGER.info("reg id {}, entity id {},idp {}.", r.getRegistrationId(), r.getEntityId(),
                            r.getAssertingPartyDetails().getEntityId());
