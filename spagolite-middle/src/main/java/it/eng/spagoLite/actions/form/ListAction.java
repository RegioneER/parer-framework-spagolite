...
public abstract class ListAction<T extends Form, U extends IUser<?>> extends FormAction<T, U> {
	...
	List<SingleValueField<Object>> list = (List<SingleValueField<Object>>) getForm()
	...
	if (lastSortingRule != null
		&& lastSortingRule.getColumnName().equalsIgnoreCase(columnName)) {
	...
	setParameters();
	listDetailNavigationOnClick(getTableName(), getNavigationEvent(), getRiga(),
		getForceReload());
    }
    ...
    // probabilmente questa � una submit
     ...
     * @param navigationEvent evento
     * @param riga            riga
     * @param forceReload     reload
     *
     * @throws EMFError eccezione generica
     */
	...
	if (navigationEvent.equalsIgnoreCase(NE_ELENCO)) {
	    ...
	    // invoco il lazyloading solo se � una tabella paginata e se sono alla prima
	    // riga
	    ...
	    // evento invocato se sto paginando dal dettaglio
	    // invoco il lazyloading solo se � una tabella paginata e se sono all'ultima
	    // riga (ie. l'ultima riga della
	    // pagina +1 � maggiore del numero di risultati relativi)
	    if (list.getTable().getLazyListInterface() != null
	...
	// MEV #33070
	    ...
	    // Se il numero � corretto (un intero e che non supera il numero di pagine
	    // disponibili)
	    ...
	    // eventuale chiamata al goToPageNavigation(list) di cui eseguire l'override
	    // nella action
	...
	// invoco l'ordinamento lazy se � una tabella paginata
	...
	BaseTableInterface<?> table = list.getTable();
	// invoco la paginazione lazy se � una tabella paginata e se la pagina
	// successiva non � contenuta
	...
	BaseTableInterface<?> table = list.getTable();
	if (table.getLazyListInterface() != null
		&& table.fullSize() > table.getLazyListInterface().getFirstResult()
	...
	BaseTableInterface<?> table = list.getTable();
	if (table.getLazyListInterface() != null
		&& table.getLazyListInterface().getFirstResult() > 0) {
    ...
    protected void listDetailNavigationOnClick(String tableName, String navigationEvent,
	    String riga, String forceReload) throws EMFError {
	List<?> list = (List<?>) getForm().getComponent(tableName);
	forwardToPublisher(getLastPublisher());
	...
	for (SingleValueField<?> field : list) {
	    if (field.isEditMode()) {
		list.getTable().getCurrentRow().setObject(field.getName(),
			list.getColumn(field.getName()).parse());
    ...
    // meccanismo di "sicurezza" per garantire che il singolo *navigationEvent*
    // risponda
