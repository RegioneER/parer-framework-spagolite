...
public class SessionManager extends SessionCoreManager {
	...
	ArrayList<ExecutionHistory> list = (ArrayList<ExecutionHistory>) httpSession
		.getAttribute(NAVHIS_CONTAINER);
	if (list == null) {
    ...
    public static ExecutionHistory setLastExecutionHistory(HttpSession httpSession,
    ...
    public static void addPrevExecutionToHistory(HttpSession httpSession, boolean isAction,
	    boolean isForward) {
	SessionManager.addPrevExecutionToHistory(httpSession, isAction, isForward, null);
    }

    public static void addPrevExecutionToHistory(HttpSession httpSession, boolean isAction,
	    boolean isForward, String params) {
	ExecutionHistory history;
	if (isAction) {
	    history = new ExecutionHistory(getCurrentAction(httpSession), getForm(httpSession),
		    isAction, isForward);
	    ...
	    history = new ExecutionHistory(getLastPublisher(httpSession), null, isAction,
		    isForward);
	}
	ArrayList<ExecutionHistory> list = getExecutionHistory(httpSession);
	list.add(history);
    ...
    public static void setAll(HttpSession httpSession, String action, Form form,
	    MessageBox messageBox, IUser user) {
