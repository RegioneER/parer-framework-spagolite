...
public class Casting {
     ...
     *
     * @param strDate data in formato string
     *
     ...
     * Trasforma la stringa passati in Timestamp
     *
     * @param strInteger data in formato integer
     *
    ...
    public static BigDecimal parseInteger(String strInteger) throws EMFError {
        ...
        // Rimosso isDigits per poter gestire il "-" dei numeri negativi che altrimenti
        // non verrebbero considerati
     ...
     *
     * @param strDecimal data in formato string
     *
     * @return BigDecimal della data in ingresso
     *
     * @throws EMFError eccezione generica
    ...
    public static String parseCodFiscalePartitaIva(String codice) throws EMFError {
        if (isCodiceFiscale(codice) || isPartitaIva(codice)) {
            ...
            throw new EMFError(EMFError.WARNING,
                    "Errore nella validazione del codice fiscale o della partita IVA");
        }
    }
    ...
    public static String parsePrefissoTelFax(String prefisso) throws EMFError {
        ...
        if (prefisso.contains("0")) {
            if (prefisso.length() == 1
                    || prefisso.length() == StringUtils.countMatches(prefisso, "0")) {
                res = false;
            }
        }
        if (!res) {
    ...
    public static final String format(Object value, Type.Enum type, String format) throws EMFError {
        try {
            if (StringUtils.isNotBlank(format)
                    && (type.equals(Type.DATE) || type.equals(Type.DATETIME))) {
     ...
     *
     * @param value oggetto generico
     * @param type  tipo {@link Enum}
     *
    ...
    public static final String format(Object value, Type.Enum type) throws EMFError {
        if (value != null) {
            if (type.equals(Type.STRING) || type.equals(Type.CAP) || type.equals(Type.EMAIL)
                    || type.equals(Type.CODFISCALE) || type.equals(Type.TELEFONO)
                    || type.equals(Type.PARTITAIVA) || type.equals(Type.CODFISCPIVA)
                    || type.equals(Type.PREFISSOTEL) || type.equals(Type.PASSWORD)) {
     ...
     *
     * @param value string in ingresso
     * @param type  tipo {@link Enum}
     *
    ...
    public static final void setToExcel(Cell cell, String value, Type.Enum type) throws EMFError {
        if (StringUtils.isNotBlank(value)) {
            if (type.equals(Type.STRING) || type.equals(Type.CAP) || type.equals(Type.EMAIL)
                ...
                if (StringUtils.isNotBlank(value)) {
                    try {
                        Number num = new DecimalFormat("", new DecimalFormatSymbols(Locale.ITALIAN))
                                .parse(value);
                     ...
                     * cell.setCellValue(Double.parseDouble(value)); } else { cell.setCellValue("");
                     * }
                     */
                } else {
    ...
    public static final void setToExcelMultiValue(Cell cell, Set<String> values, Type.Enum type)
            throws EMFError {
