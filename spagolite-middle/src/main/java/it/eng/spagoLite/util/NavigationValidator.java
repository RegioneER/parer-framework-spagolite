...
public class NavigationValidator {
     ...
     * specificato per la validazione del numero di pagina nella funzionalit� goToPage
     *
     ...
     *
     * @return Un oggetto Integer contenente il valore numerico della stringa se la stringa � un
     *         intero valido compreso tra 0 e maxVal. Restituisce null se la stringa non � un
     *         intero valido o � fuori dal range.
     */
	...
	// impostato il cui nome sar� numPag +
	// nome_lista.
	// Mi recupero un array perch� potrei avere pi� di una barra di scorrimento (una
	...
	if (str.length > 2) {
	    getMessageBox().addError(
		    "Attenzione: sono presenti pi� di 2 barre di scorrimento per la stessa lista, operazione non consentita");
	}
	...
	if (valoreNumeroPagina == null || valoreNumeroPagina.isEmpty()) {
	    ...
	    return null; // Stringa nulla o vuota non � un intero valido
	}
	...
	try {
	    ...
	    // 3. Se arriviamo qui, la stringa � un intero valido nel range
	    return num; // Restituisci l'Integer
	} catch (NumberFormatException e) {
	    getMessageBox().addError(
		    "Attenzione: il numero fornito per selezionare la pagina desiderata non � formalmente corretto");
	    // L'eccezione NumberFormatException viene lanciata se la stringa non pu� essere
