...
public class NavigationValidator {
    ...
    /**
     * Verifica se una stringa rappresenta un intero valido compreso tra 0 e un valore massimo specificato per la
     * validazione del numero di pagina nella funzionalità goToPage
     *
     * @param request
     *            contiene il valore della pagina su cui atterrare
     * @param nomeLista
     *            nome della lista
     * @param maxVal
     *            Il valore massimo consentito (incluso).
     *
     * @return Un oggetto Integer contenente il valore numerico della stringa se la stringa è un intero valido compreso
     *         tra 0 e maxVal. Restituisce null se la stringa non è un intero valido o è fuori dal range.
     */
    public Integer getValidNavigationInteger(HttpServletRequest request, int maxVal, String nomeLista) {
        // Recupero il parametro in servletRequest relativo al numero di pagina impostato il cui nome sarà numPag +
        // nome_lista.
        // Mi recupero un array perchè potrei avere più di una barra di scorrimento (una sopra e una sotto la lista)
        ...
        if (str.length == 2) {
            if (str[0].equals(str[1])) {
                valoreNumeroPagina = str[0]; // Gestisce sia il caso di entrambi vuoti sia il caso di entrambi uguali
        ...
        try {
            ...
            // L'eccezione NumberFormatException viene lanciata se la stringa non può essere convertita in un intero.
            return null; // La stringa non rappresenta un intero valido
        } catch (Exception e) {
            // Gestione di altre eccezioni impreviste (raro, ma per completezza).
            logger.error("Errore inatteso durante la validazione del numero di paginazione lista: " + e.getMessage()); // Log
                                                                                                                       // dell'errore
