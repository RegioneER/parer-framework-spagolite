...
public abstract class SingleValueField<O extends Object> extends Field {
    ...
    public SingleValueField(Component parent, String name, String description, String alias, Enum type, String format,
            boolean required, boolean hidden, boolean readonly, boolean trigger) {
        this(parent, name, description, alias, type, format, required, hidden, readonly, trigger, null);
    }

    public SingleValueField(Component parent, String name, String description, String alias, Enum type, String format,
            boolean required, boolean hidden, boolean readonly, boolean trigger, String iconUrl) {
        ...
        if ((this instanceof Input || this instanceof Link) && StringUtils.isNotBlank(htmlValue) && check()) {
            if (getType().equals(Type.INTEGER) || getType().equals(Type.DECIMAL)) {
                ...
                try {
                    ...
                    throw new EMFError(EMFError.ERROR, "Errore inatteso nel parsing del valore", ex);
                }
    ...
    public Message validate() {
        if (getType().equals(Type.CAP) && !check()) {
            return new Message(MessageLevel.ERR,
                    "Il Codice di Avviamento Postale (" + getDescription() + ") deve essere di 5 cifre");
        }

        if (getType().equals(Type.CODFISCALE) && !check()) {
            return new Message(MessageLevel.ERR,
                    "Il codice fiscale inserito (" + getDescription() + ") non &egrave; formalmente corretto");
        }

        if (getType().equals(Type.PARTITAIVA) && !check()) {
            return new Message(MessageLevel.ERR,
                    "La partita iva inserita (" + getDescription() + ") non &egrave; formalmente corretta");
        }

        if (getType().equals(Type.CODFISCPIVA) && !check()) {
            return new Message(MessageLevel.ERR,
                    "Il codice inserito (" + getDescription() + ") non &egrave; formalmente corretto");
        }

        if (getType().equals(Type.EMAIL) && !check()) {
            return new Message(MessageLevel.ERR, "L'email inserita (" + getDescription() + ") non &egrave; valida");
        }

        if (getType().equals(Type.TELEFONO) && !check()) {
            return new Message(MessageLevel.ERR,
                    "Il numero di telefono o fax (" + getDescription() + ") inserito non &egrave; valido");
        }

        if (getType().equals(Type.PREFISSOTEL) && !check()) {
            return new Message(MessageLevel.ERR,
                    "Il prefesso di telefono o fax (" + getDescription() + ") inserito non &egrave; valido");
        }

        if ((getType().equals(Type.DATE) || getType().equals(Type.DATETIME)) && !check()) {

            return new Message(MessageLevel.ERR, "Il campo (" + getDescription() + ") inserito non &egrave; valido");
        }

        if (!check()) {
            return new Message(MessageLevel.ERR, "Campo '" + getDescription() + "' formalmente errato");
        }
