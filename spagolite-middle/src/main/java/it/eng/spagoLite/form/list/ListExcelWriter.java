        ...
        for (Field field : getFields()) {
            if (!(field instanceof Button)) {
                ...
                if (!field.isHidden()) {
                    ...
                    if (field instanceof ComboBox) {
                        Casting.setToExcel(cell, ((SingleValueField<?>) field).getDecodedValue(),
                                Type.STRING);
                    } else if (field instanceof MultiSelect) {
                        Casting.setToExcelMultiValue(cell,
                                ((MultiValueField<?>) field).getDecodedValues(), Type.STRING);
                    } else {
                        Casting.setToExcel(cell, ((SingleValueField<?>) field).getValue(),
                                field.getType());
                    }
