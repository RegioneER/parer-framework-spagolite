...
public class TreeTag extends BaseFormTag<Tree<TreeField<?>>> {
    ...
    private int drawBody(BaseTableInterface<?> table, Tree<?> fields, StringBuilder body) throws JspException {
        try {
            ...
            if (StringUtils.isNotBlank(ulList)) {
                // display: none; per evitare la transazione da DOM ad oggetto gestito dal plugin jstree
            ...
            if (fields.isEditable()) {
                if (!fields.isHideCreateNodeButton()) {
                    Button createButton = new Button(fields, fields.getName() + "_createBtn", "Crea nodo", "",
                            it.eng.spagoLite.xmlbean.form.Field.Type.STRING, null, false, false, false, false, false,
                            true);
                    String action = "button/" + getForm().getClass().getSimpleName() + "#" + fields.getName() + "/"
                            + createButton.getName();
                    ...
                    if (fields.isUseDefaultCreateNodeButton()) {
                        String name = JavaScript.stringToHTMLString(StringUtils.uncapitalize(createButton.getName()));
                        body.append("\n$(\"#" + name + "\").click( function() {\n" + "if ( $(\"#" + treeId
                                + " li\").length > 0) {" + "$(\"#" + treeId + "\").jstree(\"create\");" + "\n} else {"
                                + "$(\"#" + treeId + "\").jstree(\"create\", -1);" + "\n}" + "\n});");
                    }
                }

                if (!fields.isHideDeleteNodeButton()) {
                    Button deleteButton = new Button(fields, fields.getName() + "_deleteBtn", "Elimina nodo", "",
                            it.eng.spagoLite.xmlbean.form.Field.Type.STRING, null, false, false, false, false, false,
                            true);
                    String action = "button/" + getForm().getClass().getSimpleName() + "#" + fields.getName() + "/"
                    ...
                    if (fields.isUseDefaultDeleteNodeButton()) {
                        String name = JavaScript.stringToHTMLString(StringUtils.uncapitalize(deleteButton.getName()));
                        body.append("\n$(\"#" + name + "\").click( function() {\n" + "$(\"#" + treeId
                                + "\").jstree(\"remove\");\n" + " });");
                    }
                }
                ...
                if (!fields.isHideRenameNodeButton()) {
                    Button renameButton = new Button(fields, fields.getName() + "_renameBtn", "Modifica nodo", "",
                            it.eng.spagoLite.xmlbean.form.Field.Type.STRING, null, false, false, false, false, false,
                            true);
                    String action = "button/" + getForm().getClass().getSimpleName() + "#" + fields.getName() + "/"
                            + renameButton.getName();
                    ...
                    if (fields.isUseDefaultRenameNodeButton()) {
                        String name = JavaScript.stringToHTMLString(StringUtils.uncapitalize(renameButton.getName()));
                        body.append("\n$(\"#" + name + "\").click( function() {\n" + "$(\"#" + treeId
                                + "\").jstree(\"rename\");\n" + " });");
                    }
                }
            ...
            body.append("\n$(function() {" + "$(\"#" + treeId + "\").jstree({"
                    + (StringUtils.isNotBlank(getAdditionalJsonParams()) ? getAdditionalJsonParams() + "," : "")
                    + plugins
                    + (StringUtils.isNotBlank(getAdditionalPluginParams()) ? "," + getAdditionalPluginParams() : "")
    ...
    private String getFieldButton(Button createButton) throws JspException {
        ...
        String submitName = JavaScript.stringToHTMLString(StringUtils.uncapitalize(createButton.getName()));
        String description = JavaScript.stringToHTMLString(createButton.getDescription());
        String disabled = "";
        return "<input type=\"button\" id=\"" + submitName + "\" value=\"" + description + "\" class=\"" + className
                + "\" " + disabled + " />\n";
    }
    ...
    private String buildTypes() {
        String types = "\"types\" : {" + "\"#\" : {" + "\"max_depth\" : -2,\"max_children\" : -2 " + "},"
                ...
                + "\"black\" : {" + "\"icon\" : \"" + CONTEXTPATH + "/img/jstree/Black.png" + "\"" + "},"
                + "\"blue\" : {" + "\"icon\" : \"" + CONTEXTPATH + "/img/jstree/Blue.png" + "\"" + "},"
                + "\"green\" : {" + "\"icon\" : " + "\"" + CONTEXTPATH + "/img/jstree/Green.png" + "\"" + "},"
                + "\"red\" : {" + "\"icon\" : \"" + CONTEXTPATH + "/img/jstree/Red.png" + "\"" + "}," + "\"yellow\" : {"
                + "\"icon\" : \"" + CONTEXTPATH + "/img/jstree/Yellow.png" + "\"" + "}" + "}";
        return types;
    }
    ...
    private int closeChildrens(StringBuilder ulBuilder, String parent, List<String> parentList, int level) {
