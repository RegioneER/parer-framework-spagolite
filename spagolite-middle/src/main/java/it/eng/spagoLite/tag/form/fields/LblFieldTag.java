        ...
        try {
            ...
            if (getComponent() instanceof Button) {
                ...
                if (link.isTargetAList()) {
                    ...
                    linkAction = "detail/" + getForm().getClass().getSimpleName() + "#" + link.getParent().getName()
                            + "/" + link.getTarget();
                }
                ...
                String className = ContainerTag.LEFT.equalsIgnoreCase(getControlPosition())
                        ? "slText " + getControlWidth() : "slTextRight " + getControlWidth();
                ...
                if (isUserAuthorized(linkAction)) {
                    ...
                    String linkString = "<a title=\"" + tooltip + "\" class=\"" + name + "\" href=\""
                            + getOperationUrl(link.getTarget(), "riga=-1") + "\">" + value + "</a>&nbsp;";
                    writeln(linkString);
                } else {
                    String hidden = " <input type=\"hidden\" name=\"" + name + "\" id=\"" + name + "_hidden\" value=\""
                            + value + "\" />\n";
                    writeln(hidden + " <div  id=\"" + name + "\" class=\"" + className + "\" >" + value + "</div>");
                }
            } else {
