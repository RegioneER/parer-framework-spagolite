...
public class SelectListTag extends AbstractListTag {
    ...
    /**
     * Implementa una lista di cui è possibile scegliere una riga ed eseguire un'azione con la riga
     * selezionata
     */
	...
	try {
		...
		for (SingleValueField<?> field : fields) {
		    String headerContent = isSortable() ? getSortLink(field)
		...
		for (Object object : table) {
			...
			if (i == table.getCurrentRowIndex()) {
			    className = (i % 2 == 0) ? "rigaPari rigaCorrente"
				    : "rigaDispari rigaCorrente";
			} else {
			    className = (i % 2 == 0) ? "rigaPari" : "rigaDispari";
			}

			writeln("  <tr>");
			// Controllo se è presente un filtro per cui non debba inserire l'icona di
			// aggiunta..
			String visibilityProperty = getVisibilityProperty();
			String sAction = null;
			if (addList && isAddAction()) {
			    debugAuthorization(addAction);
			    sAction = "    <td class=\"" + className + "\">"
				    + getSubmit(ListAction.NE_DETTAGLIO_SELECT, SELECT_STYLE,
					    "Seleziona riga", "", i, false)
				    + "</td>";
			    if (visibilityProperty != null) {
				boolean isSelectVisible = controlVisibilityProperty(row,
					visibilityProperty);
				sAction = isSelectVisible ? sAction
					: "<td class=\"" + className + "\"></td>";
			    }
			} else if (!addList && isRemoveAction()) {
			    debugAuthorization(removeAction);
			    sAction = "    <td class=\"" + className + "\">"
				    + getSubmit(ListAction.NE_DETTAGLIO_SELECT, DESELECT_STYLE,
					    "Rimuovi riga", "", i, false)
				    + "</td>";
			}
			...
			if (isViewAction() && !fields.isHideDetailButton()) {
				...
				writeln("   <td class=\"" + className + "\">"
					+ getLink("", ListAction.NE_DETTAGLIO_VIEW, DETTAGLIO, null,
						null, i, false, true)
					+ "</td>\n");
			    } else {
			...
			for (SingleValueField<?> field : fields) {
				...
				strValue = FieldTag.Factory.htmlField(field, " w100 center",
					ContainerTag.LEFT, getActionName(), i, false,
					StringUtils.EMPTY);
			    } else if (field instanceof CheckBox<?>) {
				field.setEditMode();
				strValue = FieldTag.Factory.htmlField(field, " w100 center",
				...
				if (isUserAuthorized(linkAction)) {
					...
					boolean error = false;
					if (StringUtils
						.isNotBlank(link.getExternalLinkParamApplic())) {
					    ...
					    String externalId = "";
					    if (StringUtils
						    .isNotBlank(link.getExternalLinkParamId())) {
					    ...
					    operationUrl = externalUrl + link.getTarget()
						    + externalId;
					} else {
			    ...
			    if (field.isHidden()) {
				write("   <td class=\"" + className
					+ " displayNone\"><input type=\"hidden\" value=\""
					+ strValue + "\" name=\"" + field.getName() + "\" /></td>");
			    } else {
				writeln("   <td  title=\"" + titleValue + "\" class=\"" + className
					+ "\">" + strValue + "</td>");
			    }
		...
		// + table.size()
		// +
		// ". Introdurre parametri di ricerca pi&ugrave; selettivi per affinare la
		// ricerca.</i></div>");
		// }
	    }
	} catch (EMFError e) {
