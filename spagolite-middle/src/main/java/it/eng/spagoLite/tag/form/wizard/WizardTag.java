...
public class WizardTag extends AbstractWizardTag {
    ...
    protected String writeStepCounter() throws JspException {
        if (!getComponent().isHideStepCounter()) {
            return " (Passo " + (getComponent().currentStepIndex() + 1) + " di "
                    + getComponent().stepSize() + ")";
        } else {
            return "";
        }
    ...
    public int doStartTag() throws JspException {
        ...
        writeln("<div>");
        writeln(" <h2 class=\"wizardTitle\">" + wizard.getDescription() + " - "
                + getCurrentStep().getDescription() + writeStepCounter());
