    ...
    private void drawHead(String styleClass, String title, List<?> fields, StringBuilder head, int rowEditable) {
        ...
        for (SingleValueField<?> field : fields) {
            ...
            if (field instanceof CheckBox && !field.isReadonly()) {
                beginth = "<th class=\"cbth\" id=\"" + JavaScript.stringToHTMLString(field.getName()) + "\">";
            }
            ...
            if (!field.isHidden()) {
                if (field instanceof Link) {
                    ...
                    if (((Link) field).isTargetAList()) {
                        ...
                        linkAction = "detail/" + getForm().getClass().getSimpleName() + "#" + ((Link) field).getTarget()
                                + "/view";
                    } else {
                    ...
                    head.append("    " + beginth
                            + (StringUtils.isNotBlank(field.getHtmlDescription()) ? headerContent : "") + "</th>\n");
                } else {
        ...
        if (!fields.isReadOnly()) {
            ...
            if (isEditAction() && isContentEditable() && !fields.isHideUpdateButton() && !isHideOperationButton()) {
                head.append("   <th></th>\n");
            }
            if (isDeleteAction() && isContentDeletable() && !fields.isHideDeleteButton() && !isHideOperationButton()) {
                head.append("   <th></th>\n");
            }
        }
            ...
            if (table.getPageSize() <= 0 || (i >= table.getFirstRowPageIndex() && i <= table.getLastRowPageIndex())) {
                ...
                if (isViewAction() && !fields.isHideDetailButton()) {
                    ...
                    if (isRowVisible(row)) {
                        body.append("   <td class=\"" + className + "\">" + getLink("", ListAction.NE_DETTAGLIO_VIEW,
                                DETTAGLIO, null, null, i, false, true, getMainNavTable()) + "</td>\n");
                    } else {
                ...
                for (SingleValueField<?> aafield : fields) {
                    ...
                    if (field instanceof ComboBox) {
                        ...
                        aafield.setEditMode();
                        field.setEditMode();
                        strValue = FieldTag.Factory.htmlField(field, " w100 center", ContainerTag.LEFT, getActionName(),
                                i, false, StringUtils.EMPTY);
                    } else if (field instanceof Link) {
                        boolean isLinkVisible = controlVisibilityProperty(row, ((Link) field).getVisibilityProperty());
                        if (isLinkVisible) {
                            String linkAction = "";
                            if (((Link) field).isTargetAList()) {
                                ...
                                if (field.isHidden()) {
                                    ...
                                    strValue = "<a title=\"" + ((Link) field).getTooltip() + "\" class=\""
                                            ...
                                            + getOperationUrl("listNavigationOnClick", "table="
                                                    ...
                                                    + "&amp;forceReload=false&amp;nomeColonna=" + field.getName())
                                            + "\">&nbsp;</a>&nbsp;";
                                } else {
                                    ...
                                    if (StringUtils.isNotBlank(link.getExternalLinkParamApplic())) {
                                        ...
                                        if (StringUtils.isNotBlank(link.getGenericLinkId())) {
                                            String genericLink = "";
                                            Object propertyValue = row.getObject(link.getGenericLinkId());
                                            if (propertyValue == null) {
                                                error = true;
                                            } else {
                                            ...
                                            operationUrl = getOperationUrl(((Link) field).getTarget(),
                                                    "table=" + getName() + "&riga="
                                                            + ((i < 0) ? "-1" : String.valueOf(i)) + "&amp;nomeColonna="
                                                            + field.getName());
                                        }
                                    }
                                    if (!error) {
                                        strValue = "<a title=\"" + (link).getTooltip() + "\" class=\"" + field.getName()
                                                + "\" href=\"" + operationUrl + "\" " + targetAttribute + ">"
                    ...
                    if (field.isHidden()) {
                        body.append("   <td class=\"" + className + " displayNone\"><input type=\"hidden\" value=\""
                                + strValue + "\" name=\"" + field.getName() + "\" /></td>\n");
                    } else {
                        body.append("   <td title=\"" + titleValue + "\" class=\"" + className + "\">" + strValue
                                + "</td>\n");
                    }
                }
                if (!fields.isReadOnly()) {
                    if (isEditAction() && !fields.isHideUpdateButton() && !isHideOperationButton()) {
                        debugAuthorization(body, editAction);
                        if (isRowEditable(row)) {
                            body.append(
                                    "   <td class=\"" + className + "\">" + getLink("", ListAction.NE_DETTAGLIO_UPDATE,
                                            UPDATE_IMG, null, null, i, false, true) + "</td>\n");
                        } else {
                            body.append("   <td class=\"" + className + "\"></td>\n");
                        }
                    }
