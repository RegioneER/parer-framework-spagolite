...
public class FieldTag extends BaseFormTag<SingleValueField<?>> {
	...
	try {
	    writeln(Factory.htmlField(getComponent(), getControlWidth(), getControlPosition(),
	...
	if (field.isTrigger()) {
	    triggers = "<input type=\"hidden\" id=\"trigger" + id + "\" name=\"trigger" + name
		    + "\" value=\"" + JavaScript.stringToHTMLString(value) + "\" />\n";
	}
	...
	private static final String htmlInput(Input<?> field, String width, String position,
		String actionName, int index, String tooltip) throws EMFError {
	    ...
	    String className = ContainerTag.LEFT.equalsIgnoreCase(position) ? "slText " + width
		    : "slTextRight " + width;
	    String maxLength = field.getMaxLength() > 0
		...
		if (field.getType().equals(Type.FILE)) {
		    ...
		    input = " <input id=\"" + name + "\" name=\"" + name + "\" class=\"" + className
			    + "\" type=\"password\" value=\"" + value + "\" " + maxLength
			    + " autocomplete=\"off\" />";
		} else {
		    input = " <input id=\"" + name + "\" name=\"" + name + "\" class=\"" + className
		...
		String hidden = " <input type=\"hidden\" name=\"" + name + "\" id=\"" + name
			+ "_hidden\" value=\"" + value + "\" />\n";
		return (hidden + " <div  id=\"" + name + "\" class=\"" + className + "\" >" + value
			+ "</div>" + makeTooltip(tooltip));
	    }

	}

	private static final String EmptyCombo(String name, String className, boolean isEditable,
		boolean isMultiple) {
	    String multiple = isMultiple ? " multiple=\"multiple\" " : "";
	    StringBuilder select = new StringBuilder(" <select id=\"" + name + "\" name=\"" + name
		    + "\" class=\"" + className + "\" " + multiple + ">\n");
	    select.append("  <option value=\"\"></option>\n");
	    select.append("</select>\n");
	    return isEditable ? select.toString() : "";
	}

	private static final String htmlCombo(ComboBox<?> field, String width, String position,
		String actionName, boolean withSearchComp, String tooltip) throws EMFError {
	    String name = JavaScript.stringToHTMLString(field.getName());
	    String className = ContainerTag.LEFT.equalsIgnoreCase(position) ? "slText " + width
		    : "slTextRight " + width;
		...
		if (!field.isReadonly() && field.isEditMode()) {
			...
			String descrizione = JavaScript
				.stringToHTMLString(combo.getDescrizione(key));
			descrizione = org.apache.commons.text.StringEscapeUtils
				.escapeHtml4(descrizione);
		    ...
		    String select = " <select id=\"" + name + "\" name=\"" + name + "\" class=\""
			    + className + "\" >" + out.toString() + "</select>";
		    /*
		     * si disattiva il trigger se presente il componente javascript (select2) che
		     * utilizza una custom function js
		     */
		    ...
		    return (" <div id=\"" + name + "\" class=\"" + className + "\" >"
			    + field.getHtmlDecodedValue() + makeTooltip(tooltip) + "</div>");
		}
	    } else {
		return EmptyCombo(name, className, !field.isReadonly() && field.isEditMode(),
			false);
	    }
	}

	private static final String htmlMultiCombo(MultiSelect<?> field, String width,
		String position, String actionName, String tooltip) {
	    String name = JavaScript.stringToHTMLString(field.getName());
	    String className = ContainerTag.LEFT.equalsIgnoreCase(position) ? "slText " + width
		    : "slTextRight " + width;
	    DecodeMapIF combo = field.getDecodeMap();
	    ...
	    if (!field.isReadonly() && field.isEditMode()) {
		String select = " <select multiple=\"multiple\" id=\"" + name + "\" name=\"" + name
			+ "\" class=\"" + className + "\">" + options + "</select>";
		if (field.isTrigger()) {
		    select = select + "\n" + getTriggersInput(field, field.getName());
		    select = select + "\n" + getNoscript(field, actionName);
		}
		...
		if (combo != null) {
		    decodedValues.append("  <ul id=\"" + name + "\" class=\"list-container "
			    + className + "\">\n");
		    for (Object key : combo.keySet()) {
			if (field.getValues() != null
				&& field.getValues().contains(key.toString())) {
			    decodedValues.append("    <li>"
				    + JavaScript.stringToHTMLString(combo.getDescrizione(key))
				    + "</li>\n");
			}
		    }
	...
	private static final String htmlButton(Button field, String width, String position,
		String actionName) {
	    ...
	    stringBuilder.append("<input type=\"submit\" name=\"" + submitName + "\" value=\""
		    + description + "\" class=\"" + className + "\" " + disabled + " />\n");
	    return stringBuilder.toString();
	}

	private static final String htmlCheckBox(CheckBox<?> field, String width, String position,
		String actionName, int index, String tooltip) {
	    String name = JavaScript.stringToHTMLString(field.getName());
	    String className = ContainerTag.LEFT.equalsIgnoreCase(position) ? "slText " + width
		    : "slTextRight " + width;
		...
		src = CONTEXTPATH + (field.isChecked() ? IMG_CHECKBOX_CHECKED_FIELD
			: (field.isWarning() ? IMG_CHECKBOX_WARNING_FIELD
				: IMG_CHECKBOX_UNCHECKED_FIELD));
	    } else {
	    ...
	    String alt = field.isChecked() ? "Selezionato"
		    : (field.isWarning() ? "Warning" : "Non selezionato");
	    String value;
	    if (index < 0) {
		if (field.isChecked()) {
	    ...
	    if (!field.isReadonly() && field.isEditMode()) {
		String input = " <input id=\"" + id + "\" class=\"" + className + "\" name=\""
		...
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.append(" <div id=\"" + id + "\" class=\"" + className
			+ "\" >&nbsp;<img src=\"" + src + "\" alt=\" " + alt + "\" />\n");
		stringBuilder.append("   <input type=\"hidden\" value=\"" + value + "\" name=\""
			+ name + "\" id=\"" + id + "_HIDDEN\"/>\n");
		// tooltip
	 ...
	 */
	private static final String htmlRadio(Radio<?> field, String width, String position,
		String actionName) {
	    ...
	    String className = ContainerTag.LEFT.equalsIgnoreCase(position) ? "slText " + width
		    : "slTextRight " + width;
	    String checked = field.isChecked() ? "checked=\"checked\" " : "";
	    String src = CONTEXTPATH
		    + (field.isChecked() ? IMG_RADIO_CHECKED : IMG_RADIO_UNCHECKED);
	    ...
	    if (!field.isReadonly() && field.isEditMode()) {
		String input = "    <input id=\"" + htmlId + "\" name=\"" + htmlName + "\" class=\""
			+ className + "\" type=\"radio\" value=\"" + htmlId + "\" " + checked
			+ "/>\n";
		if (field.isTrigger()) {
		    input = input + "\n" + getTriggersInput(field, field.getName());
		    input = input + "\n" + getNoscript(field, actionName);
	...
	private static final String htmlTextArea(TextArea<?> field, String width, String position,
		String actionName, String tooltip) throws EMFError {
	    String name = JavaScript.stringToHTMLString(field.getName());
	    ...
	    if (!field.isReadonly() && field.isEditMode()) {
		String input = " <textarea id=\"" + name + "\" name=\"" + name + "\" class=\""
			+ className + "\" " + rows + " " + cols + ">" + value + "</textarea>";
		if (field.isTrigger()) {
		    input = input + "\n" + getTriggersInput(field, field.getName());
		    input = input + "\n" + getNoscript(field, actionName);
		}
		...
		return (StringUtils.isNotBlank(value)
			? " <pre id=\"" + name + "\" class=\"" + className + "\"><code>" + value
				+ "</code></pre>" + makeTooltip(tooltip)
			: "");
	    }
	}

	private static final String htmlSemaphore(Semaphore field, String width, String position,
		String actionName) throws EMFError {
	    return field.getHtmlValue();

	}

	private static String makeHtmlCheckbox(MultiSemaphore.State enu, String name,
		boolean checked) {
	    StringBuilder stringBuilder = new StringBuilder();
	    stringBuilder.append("<label for=\"" + name + enu.intValue()
		    + "\" class=\"multiSemaphoreLabel\">" + enu.toImgHtml() + "</label><input id=\""
		    + name + enu.intValue() + "\" name=\"" + name + enu.intValue()
		    + "\" type=\"checkbox\" value=\"S\" " + (checked ? " checked=\"checked\"" : "")
		    + " />");
	    return stringBuilder.toString();
	}

	private static final String htmlMultiSemaphore(MultiSemaphore field, String width,
		String position, String actionName) throws EMFError {
	    String name = JavaScript.stringToHTMLString(field.getName());
	...
	private static final String makeTooltip(String tooltip) {
		...
		stringBuilder.append("\n"
			...
			+ " <span class=\"tooltiptext\">" + JavaScript.stringToHTMLString(tooltip)
			+ "</span></i>");
		return stringBuilder.toString();
	    } else {
	...
	public static final String htmlField(Component component, String width, String position,
		String actionName, int index, boolean withsearchcomp, String tooltip)
		throws EMFError {
	    if (component instanceof Input) {
		return htmlInput((Input<?>) component, width, position, actionName, index, tooltip);
	    }
	    if (component instanceof ComboBox) {
		return htmlCombo((ComboBox<?>) component, width, position, actionName,
			withsearchcomp, tooltip);
	    }
	    if (component instanceof MultiSelect) {
		return htmlMultiCombo((MultiSelect<?>) component, width, position, actionName,
			tooltip);
	    }
	    if (component instanceof CheckBox) {
		return htmlCheckBox((CheckBox<?>) component, width, position, actionName, index,
			tooltip);
	    }
