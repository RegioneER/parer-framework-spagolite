...
public class AbstractWizardTag extends BaseFormTag<Wizard> {
	...
	if (getComponent().isLastStep() || getComponent().isEndPage()) {
	    writeln("<span class=\"floatRight\">"
		    + getSubmit(Wizard.WizardNavigation.Save.toString(), "crud save", "Salva",
			    getCurrentElementName())
		    + "</span>");
	} else {
	    writeln("<span class=\"floatRight\">&nbsp;" + SAVE_DISABLED
		    + "&nbsp;Salva&nbsp;</span>");
	}
    }

    protected void writeCancel() throws JspException {
	writeln("<span class=\"floatRight\">" + getSubmit(Wizard.WizardNavigation.Cancel.toString(),
		"crud cancel", "Annulla", getCurrentElementName()) + "</span>");
    }

    protected void writePrev() throws JspException {
	if (getComponent().isFirstStep()) {
	    // writeln("<span class=\"floatLeft\">&nbsp;" + PREV_DISABLED +
	    // "&nbsp;Indietro&nbsp;</span>");

	} else {
	    writeln("<span class=\"floatLeft\">"
		    + getSubmit(Wizard.WizardNavigation.Prev.toString(), "crud prev", "Indietro",
			    getCurrentElementName())
		    + "</span>");
	...
	if (getComponent().isLastStep() || getComponent().isEndPage()) {
	    // writeln("<span class=\"floatLeft\">&nbsp;" + NEXT_DISABLED +
	    // "&nbsp;Avanti&nbsp;</span>");
	} else {
	    writeln("<span class=\"floatLeft\">"
		    + getSubmit(Wizard.WizardNavigation.Next.toString(), "crud next", "Avanti",
			    getCurrentElementName())
		    + "</span>");
	}
    }
     ...
     * @deprecated
     *
     * @param navigationEvent evento di navigazione
     * @param linkClass       rif. classe css
     * @param title           titolo
     * @param step            nome step
     * @param textInnerLink   testo link
     *
     ...
     */
    @Deprecated
    protected String getLink(String navigationEvent, String linkClass, String title, String step,
	    String textInnerLink) throws JspException {
	String submitName = "operation" + Values.OPERATION_SEPARATOR + "wizardNavigationOnClick"
		+ Values.OPERATION_SEPARATOR + getName() + Values.OPERATION_SEPARATOR
		+ navigationEvent;
	if (step != null) {
	...
	if (navigationEvent != null) {
	    // String href = getOperationUrl("wizardNavigationOnClick", "wizard=" +
	    // getName() + "&amp;navigationEvent="
	    ...
	    String hidden3 = "<input type=\"hidden\" name=\"navigationEvent\" value=\""
		    + navigationEvent + "\" />";
	    String hidden4 = "<input type=\"hidden\" name=\"operation\" value=\"wizardNavigationOnClick\" />";
	    return hidden1 + hidden2 + hidden3 + hidden4 + "<a " + styleClass + " name=\""
		    + submitName + "\"" + (title != null ? "title=\"" + title + "\"" : "")
		    + " href=\"" + href + "\">" + tagContent + "</a>&nbsp;";
	} else {
	    return "<span " + styleClass + ">" + tagContent + "&nbsp;</span>";
	}
    }

    protected String getSubmit(String navigationEvent, String style, String title,
	    String additionalInfo) throws JspException {
	return getSubmit(navigationEvent, style, title, title, additionalInfo);
    }

    protected String getSubmit(String navigationEvent, String style, String title, String value,
	    String additionalInfo) throws JspException {
	if (navigationEvent != null) {
	    String submitName = "operation" + Values.OPERATION_SEPARATOR + "wizardNavigationOnClick"
		    + Values.OPERATION_SEPARATOR + getName() + Values.OPERATION_SEPARATOR
	    ...
	    String button = "<input type=\"submit\" name=\"" + submitName + "\" title=\"" + title
		    + "\" value=\"" + value + "\" class=\"" + style + "\" " + "id=\"" + submitName
		    + "submit\" />\n";
	    return button;
	} else {
	    return "";
