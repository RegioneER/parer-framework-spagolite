...
public class MenuTag extends BaseSpagoLiteTag {
	...
	if (getMessageBox() == null || !getMessageBox().hasFatal()) {
	    ...
	    writeln(" <div class=\"version\">" + configSingleton.getAppName() + " "
		    + configSingleton.getStringValue(AMBIENTE.name()) + " v."
		    + configSingleton.getAppVersion() + " | " + configSingleton.getAppBuildDate()
		    + "</div>");
	    drawUserInfo();
	    drawCloseMenu();
	    closeUserInfo();
	...
	infoUtente.append("<span class=\"username\">("
		+ JavaScript.stringToHTMLString(getUser().getUsername()) + ")</span>");
	infoUtente.append("</div>");
	if (getUser().getScadenzaPwd() != null) {
	    infoUtente.append("<br />");
	    infoUtente.append(
		    "<div class=\"scadenzaPassword\"><span><b>Data scadenza password: </b></span>");
	    SimpleDateFormat sdf = new SimpleDateFormat(Format.DATE_FORMAT.DAY_FORMAT.format());
	    infoUtente
		    .append(JavaScript.stringToHTMLString(sdf.format(getUser().getScadenzaPwd())));
	    infoUtente.append("</div>");
	}
	infoUtente.append("<br />");
	if (showChangePasswordBtn) {
	    ...
	    infoUtente.append("<a href=\"" + ((HttpServletResponse) pageContext.getResponse())
		    .encodeURL("Home.html?operation=changePwd") + "\">Cambia password</a>");
	    infoUtente.append("</div>");
	    infoUtente.append("<br />");
	}
	if (getUser().getOrganizzazioneMap() != null) {
		...
		if (nome.equals("STRUTTURA")) {
		    String linkStrut = "<a href=\"Strutture.html?operation=loadStrutDaMenu\">"
			    + value + "</a>";
		    infoUtente.append(linkStrut);
		} else if (nome.equals("VERSATORE")) {
		    String linkVers = "<a href=\"Amministrazione.html?operation=loadVersDaMenu\">"
			    + value + "</a>";
