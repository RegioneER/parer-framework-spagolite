...
public class HeaderTag extends BaseTag {
    ...
    public int doStartTag() throws JspException {
        ...
        final String logoApplicazione = prepareImage(
                configSingleton.getStringValue(LOGO_APP_RELATIVE.name()),
        ...
        final String logo1 = prepareImage(configSingleton.getStringValue(LOGO_1_RELATIVE.name()),
                configSingleton.getStringValue(LOGO_1_ALT.name()),
                configSingleton.getStringValue(LOGO_1_URL.name()),
        ...
        if (getUser() != null && showHomeBtn) {
            ...
            writeln("    <a tabindex=\"0\" id=\"home-page\" class=\"floatLeft\" href=\""
                    + getHomePage(httpRequest.getContextPath())
            ...
            if (showChangeOrganizationBtn) {
                writeln("<h2><a href=\""
                        + ((HttpServletResponse) pageContext.getResponse())
                                .encodeURL("SceltaOrganizzazione.html")
                        + "\" title=\"" + changeOrganizationBtnDescription + "\">"
                        + "<img style=\"padding-right: 5px;\" src=\"" + contextPath
                        + IMG_CAMBIO_STRUT + "\" alt=\"" + changeOrganizationBtnDescription
                        + "\" title=\"" + changeOrganizationBtnDescription + "\" />"
                        + changeOrganizationBtnDescription + "</a></h2>" + " | ");
            }
            writeln("<h2><a href=\""
                    + ((HttpServletResponse) pageContext.getResponse()).encodeURL("Logout.html")
                    + "\" title=\"Logout\">" + "<img style=\"padding-right: 5px;\" src=\""
                    + contextPath + IMG_LOGOUT + "\" alt=\"Logout\" title=\"Logout\" />"
                    + "Logout</a></h2>");
            writeln("</div>");
            writeln("  </div>");
        }
    ...
    private String prepareImage(String relativePath, String alt, String url, String title,
            String contextPath, String cssClass) {
