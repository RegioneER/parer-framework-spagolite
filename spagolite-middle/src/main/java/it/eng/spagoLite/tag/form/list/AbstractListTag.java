...
public class AbstractListTag extends BaseFormTag<List<SingleValueField<?>>> {
    ...
    // "<img src=\""+CONTEXTPATH+"/img/toolBar/back.png\" title=\"Torna all'elenco\"
    // alt=\"Torna all'elenco\"
    ...
    protected void writeLink(String navigationEvent, String img, String text, String title)
            throws JspException {
        writeln(getLink("", navigationEvent, img, text, title, -1, false, true));
    }

    protected String getLink(String linkClass, String navigationEvent, String img, String text,
            String title, int riga, boolean forceReload, boolean isMasterList) throws JspException {
        return this.getLink(linkClass, navigationEvent, img, text, title, riga, forceReload,
                isMasterList, null);
    }

    protected String getLink(String linkClass, String navigationEvent, String img, String text,
            String title, int riga, boolean forceReload, boolean isMasterList, String name2)
            throws JspException {
        ...
        if (navigationEvent != null) {
            ...
            return "<a " + styleClass + " " + (title != null ? "title=\"" + title + "\"" : "")
                    + " href=\""
                    + getOperationUrl(remoteMethod, "table=" + getName()
    ...
    protected String getSubmit(String navigationEvent, String style, String title, String value,
            int riga, boolean forceReload) throws JspException {
        return getSubmit(navigationEvent, style, title, value, riga, forceReload, true);
    }

    protected String getSubmit(String navigationEvent, String style, String title, String value,
            int riga, boolean forceReload, boolean isMasterList) throws JspException {
        if (navigationEvent != null) {
            String remoteMethod = "listNavigationOnClick";
            if (!isMasterList) {
                remoteMethod = "listDetailNavigationOnClick";
            ...
            StringBuilder submitName = new StringBuilder("operation" + Values.OPERATION_SEPARATOR
                    + remoteMethod + Values.OPERATION_SEPARATOR + getName()
                    + Values.OPERATION_SEPARATOR + navigationEvent);
            submitName.append(
                    Values.OPERATION_SEPARATOR + ((riga < 0) ? "-1" : String.valueOf(riga)));
    ...
    protected String getSortLink(SingleValueField<?> field) {
        ...
        String colummName = StringUtils.isEmpty(field.getAlias()) ? field.getName()
                : field.getAlias();
        ...
        return "<a href=\""
                + getOperationUrl("listSortOnClick",
                        "table=" + getName() + "&amp;column=" + field.getName())
