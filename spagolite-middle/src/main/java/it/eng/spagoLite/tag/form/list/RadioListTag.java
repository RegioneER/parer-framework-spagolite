	...
	try {
		...
		for (Object object : table) {
			...
			for (SingleValueField<?> field : fields) {
				...
				String imageSrc = (((Radio<?>) field).isChecked())
					? FieldTag.IMG_RADIO_CHECKED
					: FieldTag.IMG_RADIO_UNCHECKED;
				String alt = (((Radio<?>) field).isChecked()) ? "Selezionato"
					: "Non selezionato";
				strValue = "<img src=\"" + imageSrc + "\" alt=\"" + alt + "\"/>";
			    } else {
			...
			if (getComponent().getStatus().equals(Status.view)) {
			    String imageSrc = (isChecked) ? FieldTag.IMG_RADIO_CHECKED
				    : FieldTag.IMG_RADIO_UNCHECKED;
			    String alt = (isChecked) ? "Selezionato" : "Non selezionato";
			    writeln("   <td class=\"" + className + "\"><img src=\"" + imageSrc
				    + "\" alt=\"" + alt + "\"/></td>");
			    writeln("   <td class=\"" + className + "\"></td>"); // cancellazione
										 // dell'elemento
			} else {
			    String checked = (isChecked) ? " checked=\"checked\" " : "";
			    writeln("   <td class=\"" + className + "\"><input type=\"radio\" "
				    + checked + " name=\"" + getName() + "_radio\" value=\""
				    + getName() + i + "\"/></td>");
			    writeln("   <td class=\"" + className + "\">"
				    + getLink("", ListAction.NE_DETTAGLIO_CONFIRM_DELETE,
					    DELETE_IMG, "", "", i, false, true)
