	...
	for (BaseRowInterface row : table) {
			...
			body.append(
				"   <td class=\"" + className + "\">"
					+ getLink("", ListAction.NE_DETTAGLIO_VIEW, DETTAGLIO, null,
						null, i, false, getComponent().isMasterList())
					+ "</td>");
		    } else {
		...
		for (SingleValueField<?> aafield : fields) {
			...
			body.append("   <td title=\"Seleziona la riga da modificare\" class=\""
				...
				+ "\n");
			body.append("   <input type=\"hidden\" value=\"" + strValue + "\" name=\""
				+ field.getName() + Values.OPERATION_SEPARATOR + i + "\" />\n");
			body.append("   </td>");
		...
		if (isEditAction() && !fields.isHideUpdateButton()) {
			...
			if (getComponent().isEditable()) {
				...
				body.append("     " + getSubmit(ListAction.NE_DETTAGLIO_UPDATE,
					UPDATE_STYLE, "Modifica", "", i, false,
					getComponent().isMasterList()) + "\n");
				body.append("   </td>");
			    } else {
				body.append("   <td class=\"" + className + "\"></td>\n");
			    }
			    ...
			    body.append("   <td class=\"" + className + "\">"
				    + getLink("", ListAction.NE_DETTAGLIO_UPDATE, UPDATE_IMG, null,
					    null, i, false, getComponent().isMasterList())
				    + "</td>");
			}
		...
		if (isDeleteAction() && !fields.isHideDeleteButton()) {
			...
			body.append("   <td class=\"" + className + "\">"
				+ getLink("", ListAction.NE_DETTAGLIO_CONFIRM_DELETE, DELETE_IMG,
					null, null, i, false, getComponent().isMasterList())
				+ "</td>");
		    } else {
			body.append("   <td class=\"" + className + "\"></td>");
		    }
