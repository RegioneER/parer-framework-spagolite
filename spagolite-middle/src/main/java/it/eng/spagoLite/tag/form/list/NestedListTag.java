...
public class NestedListTag extends AbstractListTag {
    ...
    private void drawBody(BaseTableInterface<?> table, List<?> fields, List<?> secondaryFields,
            StringBuilder body) throws JspException, EMFError {
        body.append(" <tbody>");
        int i = 0;
        ...
        for (BaseRowInterface row : table) {
            if (table.getPageSize() <= 0
                ...
                if (!fields.isHideDetailButton()) {
                    if (isRowVisible(row)) {
                        body.append("   <td class=\"" + className + "\" rowspan=\""
                                + numeroRigheNestedTable + "\">"
                                + getLink("", ListAction.NE_DETTAGLIO_VIEW, DETTAGLIO, null, null,
                                        i, false, true)
                                + "</td>\n");
                    } else {
                        body.append("   <td class=\"" + className + "\" rowspan=\""
                                + numeroRigheNestedTable + "\"></td>");
                    }
                }
                ...
                for (SingleValueField<?> aafield : fields) {
                    ...
                    String strValue = "";
                    if (aafield.isEditMode()
                            && (i == table.getCurrentRowIndex() || isMultiRowEdit())) {
                        className += " editable ";

                        body.append("   <td title=\"Seleziona la riga da modificare\" class=\""
                                + className + "\"  rowspan=\"" + numeroRigheNestedTable + "\">"
                                + FieldTag.Factory.htmlField(field, " w70", ContainerTag.LEFT,
                        ...
                        if (field.isHidden()) {
                            body.append("   <td class=\"" + className + " displayNone\"  rowspan=\""
                                    + numeroRigheNestedTable + "\"><input type=\"hidden\" value=\""
                                    + strValue + "\" name=\"" + field.getName() + "\" /></td>");
                        } else {
                            body.append("   <td class=\"" + className + "\" rowspan=\""
                                    + numeroRigheNestedTable + "\">" + strValue + "</td>");
                        }
                    }
    ...
    private void drawNestedRow(BaseRowInterface row, List<?> secondaryFields, StringBuilder body,
            String className) throws JspException, EMFError {
        ...
        for (SingleValueField<?> aafield : secondaryFields) {
            ...
            if (field.isHidden()) {
                body.append("   <td class=\"" + className
                        + " displayNone\"><input type=\"hidden\" value=\"" + strValue + "\" name=\""
                        + field.getName() + "\" /></td>");
            } else {
