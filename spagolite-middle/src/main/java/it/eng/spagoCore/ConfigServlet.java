...
public class ConfigServlet extends HttpServlet {
	...
	Stream.of(ConfigProperties.StandardProperty.values()).forEach(p -> {
	    configCache.put(p.name(),
		    getValue(config, appPrefix, p.getPropName(), p.getPropDefaultValue()));
	});
	...
	LOGGER.debug(
		"ConfigServlet::init: Init param [APP_VERSION] ottenuto dal MANIFEST.MF con il valore di [{}]",
		appVersion);
	LOGGER.debug(
		"ConfigServlet::init: Init param [APP_BUILD_DATE] ottenuto dal MANIFEST.MF con il valore di [{}]",
		appBuildDate);
	LOGGER.debug(
		"ConfigServlet::init: Init param [APP_NAME] ottenuto dal MANIFEST.MF con il valore di [{}]",
		appName);
	...
	while (propNames.hasMoreElements()) {
	    ...
	    LOGGER.trace("ConfigServlet::init: SysProp [{}] con valore [{}]", name,
    ...
    /**
     * Gestione delle configurazioni di inizializzazione. L'ordine con cui viene scelto il valore
     * del parametro è il seguente:
     * <ol>
     * <li>il parametro è presente sul web.xml;</li>
     * <li>E' presente un parametro dal nome <em>applicazione-nome_parametro</em> sulle proprietà 
     * di sistema;</li>
     * <li>E' presente un parametro dal nome <em>nome_parametro</em> sulle proprietà di
     * sistema;</li>
     * <li>viene utilizzato un valore predefinito per il parametro.</li>
     * </ol>
     *
     * @param config            ServletConfig, configurazioni della servlet definite su web.xml
     * @param applicationPrefix contesto dell'applicazione senza "/" per esempio <em>saceriam</em>
     * @param configName        nome della proprietà  supportata dallla Servlet
     *
     * @return valore della configurazione.
     */
    private String getValue(ServletConfig config, String applicationPrefix, String configName,
	    String defaultValue) {
	...
	if (configValue != null && !configValue.isEmpty()) {
	    LOGGER.debug(
		    "ConfigServlet::init: Init param [{}] definito in web.xml con il valore di [{}]",
	...
	LOGGER.debug(
		"ConfigServlet::init: Init param [{}] non definito. Utilizzo il valore predefinito [{}].",
    ...
    /**
     * Carica, internamente all'applicazione in fase di deploy, la risorsa esterna. Per
     * l'applicazione la risorsa sarà, a tutti gli effetti, interna.
     *
     * @param externalResource    percorso della risorsa esterna all'applicazione.
     * @param destinationPath     percorso <em>relativo</em> della risorsa all'interno
     *                            dell'applicazione
     * @param defaultRelativePath percorso della risorsa predefinita
     *
     * @return percorso relativo dalle risorsa
     */
    private String getRelativePathFromExternalResource(String externalResource,
	    String destinationPath, String defaultRelativePath) {
	try {
	    loadResource(externalResource, destinationPath);
	    LOGGER.debug("ConfigServlet::init: Risorsa {} correttamente caricata su {}.",
		    externalResource, destinationPath);
	    return destinationPath;

	} catch (IOException e) {
	    LOGGER.warn(
		    "ConfigServlet::init: Impossibile caricare la risorsa {}. Utilizzo il valore predefinito {}.",
    ...
    /**
     * Carica una risorsa e la mette a disposizione della webapp. Attualmente sono supportati i
     * seguenti schema:
     * <ul>
     ...
     * @param destination percorso <em>relativo</em> della risorsa su cui copiare
     */
	...
	while (initParameterNames.hasMoreElements()) {
		...
		LOGGER.warn("ConfigServlet::init: Init param [{}] con valore [{}] non supportato",
