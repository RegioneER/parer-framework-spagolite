...
public class ActionWriter {
    ...
    public ActionWriter(String actionPackageName, String actionClassName, String formPackageName,
	    String formClassName, Form form, String userPackage) {
	...
	writer.write("import " + (userPackage != null ? userPackage
		: getActionPackageName().subSequence(0, getActionPackageName().lastIndexOf(".")))
		+ ".User;\n");

	writer.write("\n");
	if (getForm().getListArray().length == 0) {
	    writer.write("public abstract class " + getActionClassName() + " extends FormAction<"
		    + getFormClassName() + ", User> {\n");
	} else {
	    writer.write("public abstract class " + getActionClassName() + " extends ListAction<"
		    + getFormClassName() + ", User> {\n");
	}
	writer.write("\n");
	...
	for (int i = 0; i < tabArray.length; i++) {
		...
		writer.write("  public abstract void tab" + tabElementArray[j].getName()
			+ "OnClick () throws EMFError;\n");
		writer.write("\n");
	    }
	}
	...
	for (Fields fields : getForm().getFieldsArray()) {
		...
		if (((Button) button).getSecure()) {
		    writer.write("  @Secure(action=\"button/" + formClassName + "#"
			    + fields.getName() + "/" + button.getName() + "\")\n");
		}
		writer.write(
			"  public abstract void " + button.getName() + "() throws EMFError;\n");
		writer.write("\n");
	    }
		...
		if (!link.getIsTargetList()) {
		    ...
		    writer.write("  @Secure(action=\"detail/" + formClassName + "#"
			    + fields.getName() + "/" + link.getTarget() + "\")\n");
		    writer.write(
			    "  public abstract void " + link.getTarget() + "() throws EMFError;\n");
		    writer.write("\n");
		}
	    }
	}
	// Genero gli stub per le operazioni dei pulsanti presenti sulle liste.I
	// pulsanti agiscono SOLO sulla riga
	...
	if (getForm().getButtonListArray() != null) {
		...
		for (Button button : list.getButtonArray()) {
		    ...
		    if (button.getSecure()) {
			writer.write("  @Secure(action=\"button/" + formClassName + "#"
				+ list.getName() + "/" + button.getName() + "\")\n");
		    }
		    writer.write("  public abstract void " + button.getName()
			    + "() throws Throwable;\n");
		    writer.write("\n");
		}
	...
	writer.append("  public abstract JSONObject trigger" + fqFieldName
		+ "OnTrigger () throws EMFError;\n");
	writer.append("  \n");
	writer.append(
		"  public void trigger" + fqFieldName + "OnTriggerNoAjax () throws EMFError {\n");
	writer.append("    trigger" + fqFieldName + "OnTrigger(); \n");
	writer.append("    forwardToPublisher(getLastPublisher()); \n");
	writer.append("  } \n");
	writer.append("  \n");
	writer.append(
		"  public void trigger" + fqFieldName + "OnTriggerAjax () throws EMFError {\n");
	...
	if (getForm().getListArray() != null && getForm().getListArray().length > 0) {
	    ...
	    String[] listOperations = new String[] {
		    "update", "select", "delete", "postLazyLoad", "filterInactiveRecords",
		    "goToPageNavigation" };
		...
		for (String operation : listOperations) {
		    writer.write("  public void " + operation + list.getName()
			    + "() throws EMFError {\n");
		    writer.write("  }\n");
		    writer.write("\n");
		}
	...
	if (getForm().getFieldsArray() != null && getForm().getFieldsArray().length > 0) {
	    ...
	    String[] fieldOperations = new String[] {
		...
		for (String operation : fieldOperations) {
		    writer.write("  public void " + operation + fields.getName()
			    + "() throws EMFError {\n");
		    writer.write("  }\n");
		    writer.write("\n");
		}
	...
	if (getForm().getNestedListArray() != null && getForm().getNestedListArray().length > 0) {
	    ...
	    for (NestedList nestedList : getForm().getNestedListArray()) {
		writer.write(
			"  // Operazioni disponibili per la lista " + nestedList.getName() + "\n");
		for (String operation : listOperations) {
		    writer.write("  public void " + operation + nestedList.getName()
			    + "() throws EMFError {\n");
		    writer.write("  }\n");
		    writer.write("\n");
		}
    ...
    private void writeNestedListOperationManager(String operation, Writer writer)
	    throws IOException {
	writer.write("\n");
	writer.write("  /**\n");
	writer.write("   * Manager per le operazioni di tipo \"" + operation + "\"\n");
	...
	writer.write("   */\n");
	writer.write(
		"  public void " + operation + "(NestedList<?> nestedList) throws EMFError {\n");
	writer.write("   ");
	for (NestedList nestedList : getForm().getNestedListArray()) {

	    writer.write(
		    " if (getForm().get" + nestedList.getName() + "().equals(nestedList)) {\n");
	...
	if (getForm().getWizardArray() != null && getForm().getWizardArray().length > 0) {
		...
		writer.write("  private void " + baseWizardMethod
			+ "OnEnter(Wizard wizard) throws EMFError {\n");
	    ...
	    for (Wizard wizard : getForm().getWizardArray()) {
		String baseWizardMethod = WordUtils.uncapitalize(wizard.getName());
		writer.write("  private boolean " + baseWizardMethod
			+ "OnExit(Wizard wizard) throws EMFError {\n");
		for (Step step : wizard.getStepArray()) {
		    String wizardName = "get" + WordUtils.capitalize(wizard.getName()) + "()";
		    String stepGetterName = "get" + step.getName() + "()";
		    String baseStepMethod = baseWizardMethod + WordUtils.capitalize(step.getName());
		...
		writer.write("      forwardToPublisher(getDefault" + wizard.getName()
			+ "Publisher());\n");
		writer.write("    }\n");
	    }
		...
		writer.write("      if (" + baseWizardMethod + "OnExit(wizard) && "
			+ baseWizardMethod + "OnSave() && wizard.hasEndPage() ) {\n");
		...
		writer.write("  public abstract boolean " + baseWizardMethod
			+ "OnSave() throws EMFError;\n");
		writer.write("\n");
		writer.write("  public abstract void " + baseWizardMethod
			+ "OnCancel() throws EMFError;\n");
		writer.write("\n");
		writer.write("  public abstract String getDefault" + wizard.getName()
			+ "Publisher() throws EMFError;\n");
		writer.write("\n");
		...
		for (Step step : wizard.getStepArray()) {
		    ...
		    writer.write("  */\n");
		    writer.write("  public abstract void " + baseStepMethod
			    + "OnEnter() throws EMFError;\n");
		    writer.write("\n");
		    writer.write("  /**\n");
		    writer.write("    OnExit: " + step.getDescription() + "\n");
		    writer.write("  */\n");
		    writer.write("  public abstract boolean " + baseStepMethod
			    + "OnExit() throws EMFError;\n");
		    writer.write("\n");
		}
	    }
