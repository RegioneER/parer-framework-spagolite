...
public class SpagoLiteTool {
    ...
    public SpagoLiteTool(String actionPath, String actionRerPath, String genPackage, String actionPackage,
            String formPackage) {
    ...
    private void writeAction(FileWriter fileWriter, String path) {
        List<File> actionFiles = FileSystemUtil.getFileList(path, "java", FileSystemUtil.EXLUDE_PATH);
        for (File file : actionFiles) {
            String fullClassName = file.getPath().substring(new File(srcPath).getPath().length() + 1);
            ...
            try {
                ...
                fileWriter.write(" <action name=\"" + ClassUtil.getConstantName(className) + "\" class=\""
    ...
    private void writePublisher(List<File> jspFiles) throws IOException {
        ...
        for (File file : jspFiles) {
            ...
            fileWriter.write(" <publisher name=\"" + ClassUtil.getConstantName(name) + "_PUBLISHER\">\n");
            fileWriter.write("  <rendering channel=\"HTTP\" type=\"JSP\" mode=\"FORWARD\">\n");
            fileWriter.write("   <resources>\n");
            fileWriter.write(
                    "    <item prog=\"0\" resource=\"" + file.getPath().substring(5).replace("\\", "/") + "\" />\n");
            fileWriter.write("   </resources>\n");
        ...
        try {
            ...
            for (File file : files) {
                ...
                ActionWriter actionWriter = new ActionWriter(getActionPackage(), actionClassName, getFormPackage(),
                        formClassName, formDocument.getForm(), null);
