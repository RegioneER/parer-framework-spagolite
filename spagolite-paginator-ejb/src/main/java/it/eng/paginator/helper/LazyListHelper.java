...
public class LazyListHelper {
    ...
    private <T extends AbstractBaseTable> T getTableBean(LazyListBean lazyListBean, Function<List, T> tansformFunc) {
        final LazyQuery lazyQuery = lazyListBean.getLazyQuery();
        List resultList;
        if (lazyQuery.isCriteriaQuery()) {
            if (lazyListBean.getCountResultSize() <= 0) {
                final TypedQuery<Long> query = entityManager.createQuery(lazyQuery.getCountCriteriaQuery());
            ...
            if (lazyListBean.getCountResultSize() <= 0) {
                Query count = createQuery(lazyQuery.getCountQueryString(), lazyQuery.getQueryParams());
                lazyListBean.setCountResultSize(Long.class.cast(count.getSingleResult()).intValue());
            }
            resultList = execQuery(lazyQuery.getQueryString(), lazyQuery.getQueryParams(), lazyListBean.getMaxResult(),
                    lazyListBean.getFirstResult(), lazyListBean.getOrderByColumnName(),
                    lazyListBean.getOrderBySortingRule());
        }
    ...
    /**
     * @param cq
     *            CriteriaQuery per recuperare i record dal DB
     * @param transformFunc
     *            Function che prende come parametro d'ingresso una List (i risultati dal DB) e restituisce il
     *            *TableBean
     *
    ...
    public <T extends AbstractBaseTable> T getTableBean(CriteriaQuery cq, Function<List, T> transformFunc) {
    ...
    private <T extends AbstractBaseTable> T getTableBean(Query query, Function<List, T> tansformFunc,
            String countDistinctField, LazyListBean lazyListBean) {
        setRecordLimits(lazyListBean, query);
        lazyListBean.setLazyQuery(new LazyQuery(QueryUtils.queryStringFrom(query),
                QueryUtils.selectToCount(query, countDistinctField), QueryUtils.extractParameters(query)));
    ...
    private <T extends AbstractBaseTable> T getTableBean(CriteriaQuery cq, Function<List, T> tansformFunc,
            LazyListBean lazyListBean) {
        lazyListBean.setResultListToTableBeanFunc(tansformFunc);
        lazyListBean.setLazyQuery(new LazyQuery(cq, CriteriaQueryUtils.countCriteria(entityManager, cq)));
        return getTableBean(lazyListBean, tansformFunc);
    }
    ...
    private List execQuery(final String queryString, Map<String, Object> params, int maxResults, int firstResult,
            String sortColumnName, int sortingRule) {
        String sortedQueryString = QueryUtils.handleOrderBy(queryString, sortColumnName, sortingRule);
    ...
    private List execQuery(final CriteriaQuery cq, int maxResults, int firstResult, String sortColumnName,
            int sortingRule) {
        CriteriaQueryUtils.handleOrderBy(cq, sortColumnName, sortingRule, entityManager.getCriteriaBuilder());
