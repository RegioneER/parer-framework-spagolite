...
public class TypeValidator {
     ...
     * @param ore_da     ore da
     * @param minuti_da  minuti da
     * @param data_a     data a
     * @param ore_a      ore a
     * @param minuti_a   minuti a
     * @param nm_data_da descrizione campo data da
     * @param nm_data_a  descrizione campo data a
     *
    ...
    private void isDateValid(Date data, BigDecimal ore, BigDecimal minuti, String nomeCampo) {
        ...
        if (ore == null) {
            ...
            if (minuti != null) {
                getMessageBox()
                        .addError("Orario " + nomeCampo + " non corretto: valore Ora assente");
            }
            // ora assente e minuti assente = OK, vengono impostati i valori di default
            // (00:00)
        } else {
            // ora presente e minuti assente
            if (minuti == null) {
                getMessageBox()
                        .addError("Orario " + nomeCampo + " non corretto: valore Minuti assente");
    ...
    public void isOrganizzazioneObbligatoria(String nmApplic, String nomeCampo, String valoreCampo,
            String valoreTipoOggetto) {
        if (!nmApplic.equalsIgnoreCase(Constants.nmApplic.SACER_IAM.name())
                && (valoreCampo == null || valoreCampo.isEmpty())) {
            if (nmApplic.equalsIgnoreCase(Constants.nmApplic.SACER_PREINGEST.name())
                    && valoreTipoOggetto.equals("Versatore")) {

            } else if (nmApplic.equalsIgnoreCase(Constants.nmApplic.SACER.name())
                    && valoreTipoOggetto.equals("Struttura")) {

            } else {
                getMessageBox().addError("Campo '" + nomeCampo + "' obbligatorio");
            }
        }
    }
