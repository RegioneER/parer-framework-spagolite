...
public class GestioneLogEventiAction extends GestioneLogEventiAbstractAction {
    ...
    public void loadDettaglio() throws EMFError {
        ...
        if (getNavigationEvent().equals(ListAction.NE_DETTAGLIO_VIEW) || getNavigationEvent().equals(ListAction.NE_PREV)
            ...
            if (getTableName().equals(getForm().getListaEventi().getName())) {
                ...
                valore = getForm().getListaEventi().getTable().getRow(riga).getString("nm_tipo_evento");
                getForm().getEventoDetail().getNm_tipo_evento().setValue(valore);
                valore = getForm().getListaEventi().getTable().getRow(riga).getString("calc_azione");
                ...
                // Legge i ruoli che aveva l'utente che ha generato l'evento che si sta visualizzando
                cercaRuoliUtente(Constants.nmApplic.SACER_IAM.name(),
                        getForm().getListaEventi().getTable().getRow(riga).getString("nm_applic"),
                        getForm().getListaEventi().getTable().getRow(riga).getString("nm_agente"), data);
                BigDecimal record = getForm().getListaEventi().getTable().getRow(riga)
                        .getBigDecimal("id_oggetto_evento");
                LogVLisAsserzioniDatiTableBean table = sacerLogWebEjb.getAsserzioniDatiByIdOggettoEvento(record);
    ...
    public void visualizzaValoriGrandi() throws EMFError {
        ...
        LogVLisAsserzioniDatiRowBean riga = (LogVLisAsserzioniDatiRowBean) getForm().getListaModifiche().getTable()
                .getCurrentRow();
    ...
    private void visualizzaDettaglioTransazioneCommon() throws EMFError {
        ...
        LogVVisEventoPrincTxRowBean record = sacerLogWebEjb.getLogVVisEventoPrincTxById(idTransazione);
        if (record != null) {
            getForm().getEventoPrincipale().getId_transazione().setValue(record.getIdTransazione().toPlainString());
            SimpleDateFormat df = new SimpleDateFormat("dd/MM/yyyy HH:mm");
            getForm().getEventoPrincipale().getDt_reg_evento().setValue(df.format(record.getDtRegEvento()));
            ...
            // record.getDsAzione());
            getForm().getEventoPrincipale().getAzione_composita().setValue(record.getString("azione_composita"));
            ...
            LogVRicEventiTableBean tabella3 = sacerLogWebEjb.getEventiByIdtransazioneExcludingEvento(idTransazione,
                    idEvento);
            if (tabella3 != null) {
            ...
            getMessageBox().addWarning("Attenzione: non esistono dettagli per la transazione [" + idTransazione + "]");
            goBack();
        }
    }
    ...
    public void loadRicercaEventi() throws EMFError {
        ...
        if (!nmApplic.equalsIgnoreCase(Constants.nmApplic.SACER_IAM.name())) {
            ...
            LogVUsrAbilOrganizTableBean usr = sacerLogWebEjb.getOrganizzazioniByApplicAndUsrId(nmApplic,
                    new BigDecimal(getUser().getIdUtente()));
            if (usr != null) {
                getForm().getFiltriRicercaEventi().getOrganizzazione()
                        .setDecodeMap(DecodeMap.Factory.newInstance(usr, "id_organiz_applic", "dl_composito_organiz"));
            }
        } else {
        ...
        if (apl != null) {
            getForm().getFiltriRicercaEventi().getTipoOggetto()
                    .setDecodeMap(DecodeMap.Factory.newInstance(apl, "id_tipo_oggetto", "nm_tipo_oggetto"));
        }
        getForm().getFiltriRicercaEventi().getTipoClasseEvento()
                .setDecodeMap(getMappaOrdinalGenericEnum("tipo_classe_evento", PremisEnums.TipoClasseEvento.values()));
        postLoad();
    ...
    public void ricercaEventi() throws EMFError {
        ...
        if (getForm().getFiltriRicercaEventi().validate(getMessageBox())) {
            ...
            BigDecimal idOrganizzazione = getForm().getFiltriRicercaEventi().getOrganizzazione().parse();
            ...
            if (getMessageBox().isEmpty()) {
                getForm().getRicercaEventiList().clear();
                LogVRicEventiTableBean tableBean = sacerLogWebEjb.getEventiByOrgTipoClasseDates(nmApplic, nmOggetto,
                        idOrganizzazione, idTipoOggetto, classeEvento, dataDa != null ? date[0] : null,
                        dataA != null ? date[1] : null, paramApplicHelper.getMaxResultRicercaLogEventiValue());
                if (tableBean != null && tableBean.isEmpty() == false) {
                    ...
                    getMessageBox().addWarning("Attenzione: non esistono eventi per i criteri selezionati");
                }
            }
        }
        forwardToPublisher(Application.Publisher.RICERCA_EVENTI);
    ...
    private void popolaIntestazioneDettaglioEvento(String ambiente, String ente, String struttura, String versatore,
            String oggetto, String tipoOggetto) {
        GestioneLogEventiForm form = getForm();
    ...
    private void popolaDettaglioEventoOggettoDaRicercaEventi(List lista) {
        ...
        popolaIntestazioneDettaglioEvento(row.getNmAmbiente(), row.getNmEnte(), row.getNmStruttura(),
                row.getNmVersatore(), row.getNmOggetto(), row.getNmTipoOggetto());
        ...
        getForm().getEventoDetail().getId_transazione().setValue(row.getIdTransazione().toPlainString());

        // Legge i ruoli che aveva l'utente che ha generato l'evento che si sta visualizzando
        cercaRuoliUtente(Constants.nmApplic.SACER_IAM.name(), row.getNmApplic(), row.getNmAgente(), data);
    ...
    private void cercaRuoliUtente(String nmApplicIam, String nmApplic, String nmAgente, Date dataRiferimento) {
        // Legge i ruoli che aveva l'utente che ha generato l'evento che si sta visualizzando
        java.util.List<String> ruoli = sacerLogHelper.findRuoliUtenteAllaData(nmApplicIam, nmApplic, nmAgente,
                dataRiferimento);
