...
public class SacerLogTimer implements JbossJobTimer {
    ...
    protected boolean isActive(String nomeApplicazione) {
        ...
        JobData jobDataDaConfrontare = new JobData(Constants.NomiJob.INIZIALIZZAZIONE_LOG.name(), nomeApplicazione);
    ...
    public void startCronScheduled(CronSchedule sched, String applicationName) {
        logger.warn("Il metodo startCronScheduled sul job {} non è stato implementato per l'applicazione {}",
                this.getJobName(), applicationName);
    }

    @Override
    public void stop(String applicationName) {
        logger.warn("Il metodo stop sul job {} non è stato implementato per l'applicazione {}", this.getJobName(),
                applicationName);
    }
