...
public class SacerLogWebEjb {
     ...
     */
    public LogVVisOggettoRowBean getDettOggettoByAppTipoOggettoId(String nomeApplicazione,
            String nomeTipoOggetto, BigDecimal idOggetto) {
        LogVVisOggetto ogg = sacerLogHelper.getDettOggettoByAppTipoOggettoId(nomeApplicazione,
                nomeTipoOggetto, idOggetto);
        if (ogg == null) {
            return null;
        } else {
     ...
     * Estrae il dettaglio da visualizzare nella form di visualizzazione log per oggetto
     */
    public LogVLisEventoOggettoTableBean getEventsByAppTipoOggettoId(String nomeApplicazione,
            String nomeTipoOggetto, BigDecimal idOggetto) {
        List<LogVLisEventoOggetto> lista = sacerLogHelper
                .getEventsByAppTipoOggettoId(nomeApplicazione, nomeTipoOggetto, idOggetto);
        ...
        for (LogVLisEventoOggetto ogg : lista) {
            ...
            riga.setString("calc_azione", getStringAzioneCalcolata(ogg.getDsGeneratoreAzione(),
                    ogg.getDsAzione(), ogg.getDsMotivoScript()));
    ...
    public String getStringAzioneCalcolata(String dsGeneratoreAzione, String nmAzione,
            String dsMotivazione) {
        if (dsGeneratoreAzione == null || nmAzione == null) {
            return "Azione scaturita da altra applicazione";
        } else {
            return dsGeneratoreAzione + "/" + nmAzione
     ...
     * Estrae il dettaglio da visualizzare nella form di visualizzazione log per oggetto
     */
    public LogVLisAsserzioniDatiTableBean getAsserzioniDatiByIdOggettoEvento(
            BigDecimal idOggettoEvento) {
        List<LogVLisAsserzioniDati> lista = sacerLogHelper
                .getAsserzioniDatiByIdOggettoEvento(idOggettoEvento);
        ...
        for (LogVLisAsserzioniDati ogg : lista) {
             ...
             *
             * Dalla vista se tipo _valore = 0 allora il valore è < 254 caratteri e quindi è un
             * campo normale. Se tipo valore = 1 il valore <=4000 caratteri ed è ancora un campo
             * normale. Se tipo valore = 2 il valore > 4000 caratteri ed un CLOB.
             *
             * Se però nel caso in cui nel tipo_valore = 1 è presente una stringa contenente un XML
             * allora il sistema deve copiare la stringa dal campo caratteri normale a quello
             * contenente il CLOB. Se invece non c'è un xml allora il valore è considerato ancora un
             * tipo caratteri normale ed appare il primo link.
             */
            if (ogg.getTipoValore() != null && ogg.getTipoValore().equals("1")) {
                if ((ogg.getDsValoreNewCampo() != null
                        && ogg.getDsValoreNewCampo().contains("<?xml"))
                        || (ogg.getDsValoreOldCampo() != null
                                && ogg.getDsValoreOldCampo().contains("<?xml"))) {
     ...
     */
    public AplVLogTiOggTableBean getTipiOggettoByAppName(String nomeApplicazione,
            String nomeTipoOggettoDaEscludere) {
    ...
    public LogVVisEventoPrincTxRowBean getLogVVisEventoPrincTxById(BigDecimal idTransazione) {
        ...
        if (rec == null) {
            ...
            row.setString("azione_composita", getStringAzioneCalcolata(rec.getDsGeneratoreAzione(),
                    rec.getDsAzione(), rec.getDsMotivoScript()));
            return row;
        }
    }
    ...
    public LogVUsrAbilOrganizTableBean getOrganizzazioniByApplicAndUsrId(String nomeApplicazione,
        ...
        List<LogVUsrAbilOrganiz> lista = sacerLogHelper
                .findUsrVAbilOrganizByApplicAndUser(nomeApplicazione, idUserIam);
        if (lista == null || lista.isEmpty()) {
            return null;
        } else {
    ...
    /*
     * Estrae la lista degli eventi di log secondo i parametri impostatati sull'interfaccia in
     * formato tableBean
     */
    public LogVRicEventiTableBean getEventiByOrgTipoClasseDates(String nmApplicazione,
            String nmOggetto, BigDecimal idOrganizzazione, BigDecimal idTipoOggetto,
            String classeEvento, Date DataDa, Date DataA, Integer maxResults) {
        CriteriaQuery cq = sacerLogHelper.findEventiByOrgTipoClasseDates(nmApplicazione, nmOggetto,
                idOrganizzazione, idTipoOggetto, classeEvento, DataDa, DataA);
    ...
    public LogVRicEventiTableBean getTableBeanFrom(List<ILogVRicEventi> lista) {
        if (lista == null || lista.isEmpty()) {
            ...
            for (ILogVRicEventi rec : lista) {
                ...
                if (rec.getNmAmbiente() != null && rec.getNmEnte() != null
                        && rec.getNmStruttura() != null) {
                    ...
                    riga.setString("nm_organizzazione",
                            rec.getNmAmbiente() + "/" + rec.getNmEnte());
                } else {
                    if (rec.getNmVersatore() != null && rec.getNmAmbiente() != null) {
                        riga.setString("nm_organizzazione",
                                rec.getNmAmbiente() + "/" + rec.getNmVersatore());
                    }
                }
                riga.setString("azione_composita", getStringAzioneCalcolata(
                        rec.getNmGeneratoreAzione(), rec.getNmAzione(), rec.getDsMotivoScript()));
                tabella.add(riga);
            }
            return tabella;
        }
    ...
    public LogVRicEventiTableBean getEventiByIdtransazioneExcludingEvento(BigDecimal idTransazione,
            BigDecimal idEvento) {
        List<LogVRicEventi> lista = sacerLogHelper
                .findEventiByIdTransazioneExcludingIdEvento(idTransazione, idEvento);
        if (lista == null || lista.isEmpty()) {
            return null;
        } else {
            LogVRicEventiTableBean tabella = new LogVRicEventiTableBean();
