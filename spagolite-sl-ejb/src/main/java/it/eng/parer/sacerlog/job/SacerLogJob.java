...
public class SacerLogJob {
    ...
    public void inizializzaLog(String nmApplic) throws Exception {
        ...
        for (AplVLogInit rec : l) {
            if (logIniziofatto == false) {
                logIniziofatto = true;
                sacerLogJobHelper.writeAtomicLogJob(nomeEffettivoJob, Constants.tiEvento.INIZIO_SCHEDULAZIONE.name());
            }
            BigDecimal idDaLoggare = null;
            try {
                Date dataInizioOggetto = new Date();
                List<BigDecimal> ids = sacerLogHelper.findAllObjectIdToInitialize(rec.getBlQueryTipoOggetto());
                ...
                logger.error("{} - Errore nell'esecuzione del job di inizializzazione log", nomeEffettivoJob, ex);
                String mex = "Errore sul tipo oggetto '" + rec.getNmTipoOggetto() + "' con id [" + idDaLoggare + "] "
                        + message;
                if (mex.length() > 1023) {
                    mex = mex.substring(0, 1023);
                }
                sacerLogJobHelper.writeAtomicLogJob(nomeEffettivoJob, Constants.tiEvento.ERRORE.name(), mex);
                throw ex;
            }
        }
        // FINE ELABORAZIONE
        ...
        if (logIniziofatto) {
            ...
            for (LogJobData jd : risultato) {
                mexBuf.append(jd.getNome()).append(" [").append(jd.getNumFotoEseguite()).append("] in ")
                        .append(jd.tempoTotale / 1000).append("s ");
            }
            long diff = dataFine.getTime() - dataInizio.getTime();
            mexBuf.append(", tempo totale: ").append(diff / 1000).append("s");
            sacerLogJobHelper.writeAtomicLogJob(nomeEffettivoJob, Constants.tiEvento.FINE_SCHEDULAZIONE.name(),
                    mexBuf.length() > 1023 ? mexBuf.substring(0, 1023) : mexBuf.toString());
        }
        logger.info("{} - fine esecuzione job - {}, tempo impiegato in ms: {}, elementi processati: {}",
                nomeEffettivoJob, dataFine, dataFine.getTime() - dataInizio.getTime(), l != null ? l.size() : 0);
    }
    ...
    public void execute(AplVLogInit rec, BigDecimal id, String nomeEffettivoJob) throws Exception {
        try {
            sacerLogEjb.log(null, rec.getNmApplic(), rec.getNmAgente(), rec.getNmAzioneCompSw(), rec.getNmTipoOggetto(),
                    id, rec.getNmCompSw(), PremisEnums.TipoAgenteEvento.EXECUTING_PROGRAM,
                    PremisEnums.TipoEventoOggetto.OUTCOME);
        } catch (Exception ex) {
            logger.error("{} - Errore sul tipo oggetto {} con id [{}]", nomeEffettivoJob, rec.getNmTipoOggetto(), id);
            throw ex;
        }
    }
    ...
    public void allineamentoLogByScript(String nomeApplicazione) throws Exception {
        Date dataInizio = new Date();
        String nomeEffettivoJob = Constants.NomiJob.ALLINEAMENTO_LOG.name() + "_" + nomeApplicazione;
        logger.info("{} - Inizio esecuzione job - {}", nomeEffettivoJob, dataInizio);
        List<Object[]> l = sacerLogHelper.getDistinctLogVLisEventoScriptByNmApplic(nomeApplicazione);
        sacerLogJobHelper.writeAtomicLogJob(nomeEffettivoJob, Constants.tiEvento.INIZIO_SCHEDULAZIONE.name());
        ...
        for (Object[] rec : l) {
            ...
            try {
                ...
                ejbPerNuoveTransazioni.allineaOggettiEstrattiDaScriptInNuovaTransazione((BigDecimal) rec[0],
                        (BigDecimal) rec[1], nomeApplicazione, (String) rec[2]);

            } catch (Exception ex) {
                String message = ExceptionUtils.getRootCauseMessage(ex);
                logger.error("{} - Errore nell'esecuzione del job di inizializzazione log", nomeEffettivoJob, ex);
                String mex = "Errore sul tipo oggetto '" + rec[0] + "' con id [" + rec[1] + "] " + message;
                if (mex.length() > 1023) {
                    mex = mex.substring(0, 1023);
                }
                sacerLogJobHelper.writeAtomicLogJob(nomeEffettivoJob, Constants.tiEvento.ERRORE.name(), mex);
    ...
    /*
     * Metodo che apre una nuova transazione e richiama il metodo di gestione evento script che mette un LOCK
     * PESSIMISTICO sui records estratti. Quando quegli eventi sono processati la transazione viene chiusa e i Lock
     * eliminati in modo da lasciare liberi altri thread di processare gli stessi oggetti.
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void allineaOggettiEstrattiDaScriptInNuovaTransazione(BigDecimal idTipoOggetto, BigDecimal idOggetto,
            String nmApplic, String nmTipoOggetto) throws Exception {
        logger.info("Allineo e metto il lock su: idTipoOggetto[{}] idOggetto:[{}]", idTipoOggetto, idOggetto);
        sacerLogEjb.gestisciLogEventoScript(null, nmApplic, nmTipoOggetto, idTipoOggetto, idOggetto);
        /*
